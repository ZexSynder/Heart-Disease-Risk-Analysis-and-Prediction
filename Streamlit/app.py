...
def prediction(Age,Sex,ChestPainType,RestingBP,Cholesterol,FastingBS,RestingECG,MaxHR,ExerciseAngina,Oldpeak,ST_Slope):   
    ...
    # Making predictions 
    # Skalakan data menggunakan scaler
    newscaler = scaler.transform([[Age, Sex, ChestPainType, RestingBP, Cholesterol, FastingBS,
                                RestingECG, MaxHR, ExerciseAngina, Oldpeak, ST_Slope]])

    # Lakukan prediksi menggunakan classifier
    prediction = classifier.predict(newscaler)[0]
...
def main():       
    ...
    if page == "DataSet":
        ...
        st.write('1. **Umur (Age):** Umur pasien [tahun]')
        st.write('2. **Jenis Kelamin (Sex):** Jenis kelamin pasien [M: Male, F: Female]')
        st.write('3. **Jenis Nyeri Dada (ChestPainType):** Jenis nyeri dada [TA: Typical Angina, ATA: Atypical Angina,NAP: Non-Anginal Pain, ASY: Asymptomatic]')
        st.write('4. **Tekanan Darah (RestingBP):** Tekanan darah [mm Hg]')
        st.write('5. **Kolesterol (Cholesterol):** Kolesterol [mm/dl]')
        st.write('6. **Gula Darah (FastingBS):** Gula darah [1: if FastingBS > 120 mg/dl, 0: Else]')
        st.write('7. **Elektrokardiogram (RestingECG):** Hasil elektrokardiogram [Normal: Normal, ST: having ST-T wave abnormality (T wave inversions and/or ST elevation or depression of > 0.05 mV), LVH: showing probable or definite left ventricular hypertrophy by Estes criteria]')
        st.write('8. **Denyut Jantung Maksimal (MaxHR):** Denyut jantung maksimal yang dicapai [Numeric value between 60 and 202]')
        st.write('9. **Angina (ExerciseAngina):** rasa nyeri atau ketidaknyamanan pada dada. [Y: Yes, N: No]')
        st.write('10. **Depresi (Oldpeak):** Depresi = ST [Nilai numerik diukur dalam depresi]')
        st.write('11. **Kemiringan ST (ST_Slope):** Kemiringan segmen ST [Up: upsloping, Flat: flat, Down: downsloping]')
        st.write('12. **Penyakit Jantung (HeartDisease):** Kelas keluaran [1: heart disease, 0: Normal]')
